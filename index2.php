...
if(isset($_POST["submit"])&&$_FILES["fileToUpload2"]['error']==0) {
        ...
        if($pic_uploaded2==true) {
                ...
                $newpic=new Picture();
                $newpic->name=basename(str_replace('.', $time1212.'.', $_FILES["fileToUpload2"]["name"]));
                $newpic->time=$itime2;
                $tabls0[$picstidx][]=$newpic;
                $myd->dtables=$tabls0;
                $myd->saveobjecttable("Picture");
                $currenti->dpic=$newpic->name;
...
if(!empty($_POST['text1'])&&$addline==true) {
 if (strlen(trim($msg1)) != 0&&$pic_uploaded==true) {
         if($texists==false) {
           $aone=array();
           $ftype = strtolower(pathinfo("uploads/".$newpic->name,PATHINFO_EXTENSION));
